            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                foreach (KeyValuePair<string, EncodeJob> fileNameAndJob in jobFilesWithJobs.Where(kp => kp.Value.ChunkInterval == null))
                {
                    try
                    {
                        ...
                        { throw new ApplicationException("Unable to add " + fileNameAndJob.Value + " to queue in database " + AppConfigManager.Instance.DBTypeAndString.Key); }
                    }
                ...
                foreach (string masterVideo in jobFilesWithJobs.Select(kp => kp.Value.VideoFileName).Distinct())
                {
                    ...
                    try
                    {
                        foreach (var fileNameAndJob in relevantJobs)
                        {
                            ...
                            { throw new ApplicationException("Unable to add " + fileNameAndJob.Value.ToString() + " to queue in database " + AppConfigManager.Instance.DBTypeAndString.Key); }
                        }
